...
CHUNKER_TOKENIZER_DIR = OFFLINE_MODELS_DIR / "sentence-transformers--all-MiniLM-L6-v2"
...
def build_chunker() -> HybridChunker:
    if not CHUNKER_TOKENIZER_DIR.is_dir():
        raise RuntimeError(
            "Tokenizer local introuvable pour HybridChunker: "
            f"{CHUNKER_TOKENIZER_DIR}. "
            "Miroir requis du repo Hugging Face "
            f"{CHUNKER_TOKENIZER_REPO} dans docling_models/ pour un usage hors-ligne."
        )

    tokenizer = HuggingFaceTokenizer.from_pretrained(
        model_name=CHUNKER_TOKENIZER_DIR,
        max_tokens=512,
    )
    logger.info("Utilisation du tokenizer local pour HybridChunker: %s", CHUNKER_TOKENIZER_DIR)
...
def process_file(source_path: str, results_dir: Path):
    ...
    # 2. Chunking Hybride (respecte structures, tableaux, listes)
